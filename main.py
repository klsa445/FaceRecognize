...
import numpy as np
import warnings
from mouth_open_test import mouth
from eye_blink_test import eye
...
class MainWindow(QMainWindow,Ui_MainWindow):
    ...
    def photo_img(self):
        ...
        # 保存拍照时刻的人脸
        method = random.choice([mouth, eye])
        

        if method == mouth:
            if mouth(self.cap):
                QMessageBox.about(self, '口型检测结果', '嘴巴张开，请继续操作')
                # 存储照片
                self.person_img = self.cv_img.copy()
                ...
                QMessageBox.about(self, '活体检测', '嘴巴未张开，请重新尝试')
                return
        elif method == eye:
            if eye(self.cap):
                QMessageBox.about(self, '活体检测', '眼睛闭合，请继续操作')
                # 存储照片
                self.person_img = self.cv_img.copy()
                self.boxes = self.boxes.copy()
                ...
                QMessageBox.about(self, '活体检测', '眼睛未闭合，请重新尝试')
                return
